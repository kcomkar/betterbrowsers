 ...
 *
 */
public class GJUserUtils {
	private static Logger log = Logger.getLogger(GJUserUtils.class);
	...
	public static Connection getDBConnection() throws AuthenticationException,
		...
		try {
			...
			return (( com.sap.dcm.mobile.security.GJHanaAuthenticationProvider.GJAuthentication)a).connection;
		} 
		finally{
	 ...
	 */ 
	public static GJHanaUserDetails getPrincipal() throws AuthenticationException {
		...
		if (o instanceof GJHanaUserDetails) {
			return ((GJHanaUserDetails) o);
		} else {
			throw new AuthenticationException(
					AuthenticationException.TYPE.NO_VALID_PRINCIPAL);
		}
	...
	public static boolean hasRole(String role) throws AuthenticationException {
		...
		GJHanaUserDetails p = getPrincipal();
