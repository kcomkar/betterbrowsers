App.registerPage("note", function() {
    var initialize = function(companyCode, customerId) {
        ...
        noteView.fetchData = function(url) {
            /*
             * $.getJSON("ajax/"+ url +".json", function (data) { originalData = data.noteListResponse;
             * proxy.trigger("data", originalData); });
             */

            $.ajax({
                type : 'GET',
                url : "rest/mobile/collectionOverview/getCustomer/" + companyCode + "/" + customerId + "/notes",
                dataType : 'json',
        ...
        noteView.bind("action", function() {
            /*
             * POST HEADER: content-type: application/x-www-form-urlencoded BODY: contact=wangchenchang&text=aaaaaaaa
             * 
             */
            if (actionButton.hasClass("add")) {
                ...
                $.ajax({
                    ...
                    url : "rest/mobile/collectionOverview/getCustomer/" + companyCode + "/" + customerId + "/notes",
                    data : {
                        "contact" : "wangchenchang",
                        "text" : noteView.$("textarea").val()
                    },
                    dataType : 'json',
                    success : function(data) {
                        noteView.fetchData("notes");
